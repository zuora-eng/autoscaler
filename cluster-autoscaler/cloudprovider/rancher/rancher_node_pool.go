...
func (nodePool *NodePool) Create() (cloudprovider.NodeGroup, error) {
...
func (nodePool *NodePool) Nodes() ([]cloudprovider.Instance, error) {
	nodePoolNodes, err  := nodePool.rancherManager.GetNodePoolNodes(nodePool)
	instances:= make([]cloudprovider.Instance, len(nodePoolNodes))

	for index, node := range nodePoolNodes {
		instances[index] = cloudprovider.Instance{Id: node }
	}
...
func (nodePool *NodePool) TemplateNodeInfo() (*schedulernodeinfo.NodeInfo, error) {
